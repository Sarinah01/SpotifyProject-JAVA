...
public class MusicAppFunctionalities {
    ...
    private static int getValidatedInt() {
        while (true) {
            try {
                ...
                System.out.print(cc.RED_BOLD_BRIGHT+"Invalid input.Characters are not allowed, Please enter a valid number: "+cc.RESET);
            }
        }
    }
    ...
    public static void showSongfunctions() {
        while (true) {
            ...
            System.out.println("6. Download this song");
            System.out.println("7. Like / Rate / Add to Playlist");
            System.out.println("8. Show Liked Songs");
            System.out.println("9. Show Song Ratings");
    ...
    public static void handleUserChoice(int choice) {
        switch (choice) {
            case 1 :
                pauseSong();
                System.out.println(cc.BLUE+"Song Paused.");
                break;
            case 2 :
                resumeSong();
                System.out.println(cc.BLUE+"Song Resumed.");
                break;
            case 3 :
                playNextSong();
                break;
    ...
    public static void playSong(String path) {
        try {
            ...
            playThread = new Thread(() -> {
                try {
                    System.out.println(cc.CYAN+"Playing: " + path);
    ...
    private static void playNextSong() {
        if (playlist.isEmpty()) {
            System.out.println(cc.BLUE+"Playlist is empty."+cc.RESET);
            return;
        }
    ...
    private static void playPreviousSong() {
        ...
        addToRecentlyPlayed(currentUserId, getSongIdByPath(playlist.getByIndex(currentSongIndex)));
        System.out.println(cc.BLUE+"Playing Previous Song.");
        playSong(playlist.getByIndex(currentSongIndex));
    }
        ...
        try {
            ...
            while (true) {
                try {
                    ...
                    System.out.println(cc.RED_BOLD_BRIGHT+"Enter ratings in numerical"+cc.RESET);
                    sc.nextLine();
                }
            }
            sc.nextLine();
    ...
    public void searchSongByType(String songType) {
        try {
            ...
            if (rs.next()) {
                ...
                System.out.println(cc.CYAN+"Playing Song of Given type: " + title);
                playlist.addToList(path);
                currentSongIndex=0;
                addToRecentlyPlayed(currentUserId, rs.getInt("song_id"));
                playSong(path);
            } else {
                System.out.println(cc.YELLOW+"No song found for the given criteria.");
                MusicApp1.appMainMenu();
            }
        } catch (SQLException e) {
            System.out.println(cc.RED_BOLD_BRIGHT+"Error fetching song by type: " + e.getMessage());
            MusicApp1.appMainMenu();
        }
    }
